...
void reconnectMQTT()
{
    ...
    if (!mqttClient.connected())
    {
        ...
        if (mqttClient.connect(hostname.c_str(), mqttUsername.c_str(), mqttPassword.c_str())) {
            ...
            mqttClient.subscribe((hostname + "/target_temperature/set").c_str());
            mqttClient.subscribe((hostname + "/mode/set").c_str());
            mqttClient.subscribe((hostname + "/fan_mode/set").c_str());
...
void publishHomeAssistantDiscovery()
{
    if (mqttEnabled)
    {
        ...
        mqttClient.publish((hostname + "/availability").c_str(), "online", true);        // Debug log for payload
    ...
    else
    {
        ...
        mqttClient.publish(("homeassistant/climate/" + hostname + "/config").c_str(), "");
        mqttClient.publish((hostname + "/availability").c_str(), "offline", true);
    }
}
...
void mqttCallback(char* topic, byte* payload, unsigned int length)
{
    ...
    if (String(topic) == (hostname + "/mode/set"))
    {
    ...
    else if (String(topic) == (hostname + "/fan_mode/set"))
    {
        if (message != fanMode)
        {
            fanMode = message;
    ...
    else if (String(topic) == (hostname + "/target_temperature/set"))
    {
...
void sendMQTTData()
{
    ...
    if (mqttClient.connected())
    {
        ...
        {
            mqttClient.publish((hostname + "/current_temperature").c_str(), String(currentTemp).c_str(), true);
        ...
        {
            mqttClient.publish((hostname + "/current_humidity").c_str(), String(currentHumidity).c_str(), true);
        ...
        {
            mqttClient.publish((hostname + "/target_temperature").c_str(), String(setTempHeat).c_str(), true);
            lastSetTempHeat = setTempHeat;
        }
        else if (thermostatMode == "cool" && setTempCool != lastSetTempCool)
        {
            mqttClient.publish((hostname + "/target_temperature").c_str(), String(setTempCool).c_str(), true);
            lastSetTempCool = setTempCool;
        }
        else if (thermostatMode == "auto" && setTempAuto != lastSetTempAuto)
        {
            mqttClient.publish((hostname + "/target_temperature").c_str(), String(setTempAuto).c_str(), true);
        ...
        {
            mqttClient.publish((hostname + "/mode").c_str(), thermostatMode.c_str(), true);
        ...
        {
            mqttClient.publish((hostname + "/fan_mode").c_str(), fanMode.c_str(), true);
            lastFanMode = fanMode;
        }

        // Publish availability
        mqttClient.publish((hostname + "/availability").c_str(), "online", true);
    }
}
